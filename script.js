const terminal = document.getElementById('terminal');
    ...
    // GÜVENLİK YAMASI: HTML Temizleme Fonksiyonu
    // Kullanıcının girdiği <, >, & gibi karakterleri zararsız hale getirir.
    ...
    input.addEventListener("keydown", (e) => {
        if (e.key === "Enter") {
            ...
            // GÜVENLİK YAMASI 1: Kullanıcı komutunu escapeHTML ile temizle
            output.innerHTML += `\n> ${escapeHTML(command)}`;

            // args değişkeni -game guess için gerekli olabilir (orijinal kodda eksikti ama tanımlamak iyi olur)
            const args = command.split(' ');

            // Basit komutlar
            switch (command.toLowerCase().split(' ')[0]) { // split ile ilk kelimeyi al, böylece argümanlı komutlar çalışır
                case "-help":
                    output.innerHTML += `
\nAvailable commands:
...
`;
                ...
                case "-guess":
                    // Not: Orijinal kodda 'args' tanımlı değildi, yukarıda düzelttik.
                    const guess = args[1] ? parseInt(args[1]) : null; 
...
`;
                ...
                case "-clear":
                    ...
                    break;
                default:
                    // GÜVENLİK YAMASI 2: Bilinmeyen komutları ekrana basarken de temizle
